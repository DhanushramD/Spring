...
public class LibraryService {
    ...
    public int duefine(@PathVariable int customerId, @PathVariable int ISBN) {
        return borrowRepository.overfine(customerId,ISBN);
    }
    public List<BorrowModel.overdue1> low(){
    List<BorrowModel> borrowModels = borrowRepository.overDue();
        List<BorrowModel.overdue1> o = new ArrayList<>();
    int count=0;
    for(BorrowModel borrowModel : borrowModels) {
        for (BorrowModel borrowModel1 : borrowModels){
            if(borrowModel1.getCustomerId() == borrowModel.getCustomerId()){
//                o.add(borrowModel1.getCustomerId(),count);
            }
        }
    }
        return o;
